...
public:
    ...
    {
        std::string file_directory=output_directory+"/"+STRING_UTILITIES::Value_To_String(frame);
        ...
        for(SPGrid_Block_Iterator<T_MASK> iterator(blocks.Get_Blocks());iterator.Valid();iterator.Next()){
            ...
            if(((*reinterpret_cast<const unsigned*>(&flags(offset)))&Elasticity_Node_Type_Active)||
               ((*reinterpret_cast<const unsigned*>(&flags(offset)))&Elasticity_Node_Type_Dirichlet)) ++n_nodes;}
        
        output.write((char*)&n_nodes,sizeof(unsigned long));
        LOG::cout<<"Writing "<<n_nodes<<" number of nodes."<<std::endl;
        const T_INDEX& block_size=flag_field.allocator->Block_Size().template Cast<T_INDEX>();
        for(SPGrid_Block_Iterator<T_MASK> iterator(blocks.Get_Blocks());iterator.Valid();iterator.Next_Block()){
            ...
            for(RANGE_ITERATOR<d> iterator(RANGE<T_INDEX>(base_index,base_index+block_size-1));
                ...
                if(((*reinterpret_cast<const unsigned*>(&flags(offset)))&Elasticity_Node_Type_Active)||
                   ((*reinterpret_cast<const unsigned*>(&flags(offset)))&Elasticity_Node_Type_Dirichlet)){
                    TV u;
                    for(int v=0;v<d;++v) u(v+1)=u_fields[v].Get_Const_Array()(offset);                   
                    const TV x=TV(iterator.Index())*dx+u;
                    for(int v=1;v<=d;++v) output.write((char*)&x(v),sizeof(T));}}}
